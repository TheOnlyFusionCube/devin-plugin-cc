...
const MAX_UNTRACKED_INLINE_FILES = 40;
const MAX_UNTRACKED_INLINE_TOTAL_BYTES = 256 * 1024;
...
// Inline untracked contents up to a file-count and total-byte budget; beyond
// that, name-only entries so huge new trees don't blow up the prompt.
function formatUntrackedFiles(cwd, files) {
  const parts = [];
  let inlinedBytes = 0;
  files.forEach((file, index) => {
    if (index >= MAX_UNTRACKED_INLINE_FILES || inlinedBytes >= MAX_UNTRACKED_INLINE_TOTAL_BYTES) {
      parts.push(`### ${file}\n(not inlined: beyond the untracked-content cap)`);
      return;
    }
    const rendered = formatUntrackedFile(cwd, file);
    inlinedBytes += Buffer.byteLength(rendered, "utf8");
    parts.push(rendered);
  });
  return parts.join("\n\n");
}
